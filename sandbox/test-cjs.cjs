...
async function main() {
  ...
  await mergeImages(images, `${OUTPUT_DIR}/cjs-vertical.jpg`);
  console.log('OK: vertical merge');

  // Horizontal merge.
  await mergeImages(images, `${OUTPUT_DIR}/cjs-horizontal.jpg`, {direction: 'horizontal'});
  console.log('OK: horizontal merge');

  // Grid layout.
  await mergeImages(
    [...images, `${INPUT_DIR}/red-square.jpg`],
    `${OUTPUT_DIR}/cjs-grid.jpg`,
    {grid: 2},
  );
